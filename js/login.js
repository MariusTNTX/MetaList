...
function showUserOptions(show){
  if(show){
    ...
    if(sessionStorage.getItem("foto")=="null" || sessionStorage.getItem("foto")=="") document.getElementById("userImgMini").src = defaultFoto;
...
async function login(email="", pass=""){
  ...
  if(email.length>0 && pass.length>0){
    let usuarios = await list("usuarios",true,["emailUsuario",email],['nivelPermisosUsuario_Min',1]);
    ...
    if(usuarios.response.length==1){
      ...
      if(passVerif[0].coincidence){
        if(passVerif[0].verify){
          ...
          if(usuario.foto!=null && usuario.foto.length>0) sessionStorage.setItem("foto",usuario.foto);
