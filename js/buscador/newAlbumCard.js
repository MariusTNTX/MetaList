  ...
  let txt = `
      <div class="card h-100 shadow-sm bg-light">
        ...
        <div class="card-body text-center">
          <h5 class="card-title"><a href="visor.html?element=album&band=${encodeURIComponent(album.banda)}&album=${encodeURIComponent(album.album)}">${album.album}</a></h5>
          <h6 class="card-title"><a href="visor.html?element=band&band=${encodeURIComponent(album.banda)}">${album.banda}</a></h6>
          <hr>
          <h6 class="card-title">${(album.tipo)?tipoEsp[album.tipo.replaceAll(" ","_")]:"Tipo Desconocido"}</h6>
          <h6 class="card-title">${(album.dia)?album.dia+" de ":""}${(album.mes)?mesEsp[parseInt(album.mes)-1]+" de ":""}${(album.anio)?album.anio:"Fecha Desconocida"}</h6>
