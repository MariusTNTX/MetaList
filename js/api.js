...
// GET_INFO_BANDA: FUNCIÓN OBTENER INFORMACIÓN DE LA API
async function getInfoBanda(band){
  console.log(`${path}/php/api.php?key=${key()}&select=infoBanda&nombreBanda=${band}`)
  let response = await fetch(`${path}/php/api.php?key=${key()}&select=infoBanda&nombreBanda=${band}`,{
    method: 'GET',
