      ...
      if(isset($_GET['select'])){
        ...
        if($_GET['select']=='infoBanda'){
          ...
          $consultas = [
            ...
            'albumes'=>'SELECT NomAlb as nombre, Descrip, Imagen, TipoAlb as tipo, EnLista, Anio, Mes, Dia, NumEscuchasmax as escuchas, LinkSpotify, LinkAmazon FROM albumes WHERE NomBan LIKE "'.$value.'" ORDER BY Anio',
            'generos'=>'SELECT NomGen as nombre, Estrellas FROM generos_bandas WHERE NomBan LIKE "'.$value.'"'
          ];
          ...
          foreach($data['albumes'] as $i => $album){
            if (!$resp = mysqli_query($c1, 'SELECT NomCan as nombre, Estrellas FROM canciones_albumes WHERE NomBan LIKE "'.$value.'" AND NomAlb LIKE "'.$album['nombre'].'"')){
              echo mysqli_error($c1).'<br>';
              echo 'Consulta: '.$consulta;
              exit(-1);
            } 
            $data['albumes'][$i]['canciones'] = mysqli_fetch_all($resp, MYSQLI_ASSOC);
            if (!$resp = mysqli_query($c1, 'SELECT NomGen as nombre, Estrellas FROM generos_albumes WHERE NomBan LIKE "'.$value.'" AND NomAlb LIKE "'.$album['nombre'].'"')){
              echo mysqli_error($c1).'<br>';
              echo 'Consulta: '.$consulta;
              exit(-1);
            }
