...
let tablas = ['albumes','albumes_favoritos','albumes_plus','articulos','bandas','bandas_favoritas',
              ...
              'discograficas_plus','estudios_albumes','estudios_grabacion','etapas_bandas','etiquetas_articulos',
              'generos','generos_albumes','generos_bandas','musicos','musicos_bandas','musicos_plus',
...
mdlExportCleanNext.addEventListener("click", async ()=>{
  if(currentModal=='btnExportCSV'){
    ...
    try {
      ...
      if(res[0].status==500){
          showAlert('ERROR','Ha habido un error al tratar de vaciar la tabla. Verifica las claves foráneas');
          console.error(res[0].error);
      } 
      else showAlert('SUCCESS','La tabla "'+tabla+'" ha sido vaciada con éxito');
    } catch (error) {
