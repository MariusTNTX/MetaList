...
if(isset($_GET['key'])){
    $keyResult=checkTime($_GET['key']);
  if(checkTime($_GET['key'])['status'] || $_GET['key']==$apikey){
    try {
      /* S E L E C T S ---------------------------------------------------------------------------------------------------- */
      if(isset($_GET['select'])){
        //ALMACENAMIENTO DE PARÁMETROS
        $value = (strlen($_GET['nombreBanda'])>0) ? $_GET['nombreBanda'] : [];

        //INFO_BANDA
        if($_GET['select']=='infoBanda'){
          //Se forma la raíz de la consulta
          $consultas = [
            'info'=>'SELECT NomBan as nombre, Pais, Origen, NumEscuchasmes as escuchas, Imagen, Estatus, Descrip, Linkweb, LinkSpotify FROM bandas WHERE NomBan LIKE "'.$value.'"',
            'albumes'=>'SELECT NomAlb as nombre, Descrip, Imagen, TipoAlb as tipo, EnLista, Anio as anio, Mes, Dia, NumEscuchasmax as escuchas, LinkSpotify, LinkAmazon FROM albumes WHERE NomBan LIKE "'.$value.'" ORDER BY Anio',
            'generos'=>'SELECT NomGen as nombre, Estrellas as estrellas FROM generos_bandas WHERE NomBan LIKE "'.$value.'"'
          ];
          ...
          //RECORRER ÁLBUMES (CANCIONES Y GÉNEROS)
          foreach($data['albumes'] as $i => $album){
            if (!$resp = mysqli_query($c1, 'SELECT NomCan as nombre, Estrellas as estrellas FROM canciones_albumes WHERE NomBan LIKE "'.$value.'" AND NomAlb LIKE "'.$album['nombre'].'"')){
              echo mysqli_error($c1).'<br>';
              echo 'Consulta: '.$consulta;
              exit(-1);
            } 
            $data['albumes'][$i]['canciones'] = mysqli_fetch_all($resp, MYSQLI_ASSOC);
            if (!$resp = mysqli_query($c1, 'SELECT NomGen as nombre, Estrellas as estrellas FROM generos_albumes WHERE NomBan LIKE "'.$value.'" AND NomAlb LIKE "'.$album['nombre'].'"')){
              echo mysqli_error($c1).'<br>';
              echo 'Consulta: '.$consulta;
              exit(-1);
            }
        ...
        $email = $_REQUEST['sendVerifyEmail'];
        $code = $_REQUEST['code'];
        if(count(str_split($code))>0 && count(str_split($email))>0){
            include '../phpmailer.php';
            $body = getVerifyEmailHTML($code);
            $data = sendEmail($hostEmail, $hostPass, $hostEmail, 'MetaList', $email, "Desconocido", "Verifica tu cuenta de MetaList", $body);
        } else $data[0]['error'] = "Se requieren los campos email y código";
        header("Content-type: application/json; charset=utf-8");
        echo json_encode($data);
        ...
        if(count($usuario)==1){
          ...
          include("../phpmailer.php");
          $body = getNewPassHTML($usuario,$password);
          $data = sendEmail($hostEmail, $hostPass, $hostEmail, 'MetaList', $email, $usuario, 'Nueva Contraseña', $body);
          $data[0]['coincidence'] = true;
          $data[0]['user'] = $usuario;
        ...
        include("../phpmailer.php");
        $data = sendEmail($hostEmail, $hostPass, $hostEmail, $_REQUEST['nombre'], $hostEmail, 'MetaList', $_REQUEST['asunto'], 'FROM: '.$_REQUEST['nombre'].'&lt;'.$_REQUEST['email'].'&gt;<br><br>'.$_REQUEST['mensaje']);
        header("Content-type: application/json; charset=utf-8");
        echo json_encode($data);
        ...
        $c1 = mysqli_connect($dbhost,$dbuser,$dbpass,$dbname) or die ('Error de conexion a mysql: ' . mysqli_error($c1).'<br>');
        try {
          $resultado = mysqli_query($c1,"DELETE FROM $tabla WHERE 1=1");
          if(!$resultado){
            $data[0]['status'] = 500;
            $data[0]['error'] = mysqli_error($c1);
          }
        } catch (\Throwable $th) {
          $data[0]['status'] = 500;
          $data[0]['error'] = $th;
      ...
      $data = array('status'=>$keyResult['status'],'initDate'=>$keyResult['initDate'],'endDate'=>$keyResult['endDate'],'dif'=>$keyResult['dif'],'message'=>"Error: Key no válida");
    header("Content-type: application/json; charset=utf-8");
    echo json_encode($data);
  }
} else {
